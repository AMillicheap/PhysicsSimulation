...
export function setupScene() {
    ...
    if (masses.length <= 3 && scene.showAnalyticSolution)
...
export function toggleSolution() {
    scene.showAnalyticSolution = !scene.showAnalyticSolution;
    var button = document.getElementById("toggleAnalyticSolution");
    button.innerText = scene.showAnalyticSolution ? "Turn Analytic Solution Off" : "Turn Analytic Solution On";
}
...
function draw() {
    ...
    if (scene.numSubSteps >= 100) {
        if (scene.pendulumAnalytic && scene.showAnalyticSolution)
